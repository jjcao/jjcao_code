...
int pointNum = 8; //jjcao
		std::vector<float> verts(pointNum*3);
...
int main(int argc, char **argv)
{
    ...
    {
        int bufferSize;		
		for (int i = 0; i < pointNum*3; ++i)//jjcao
		{
			verts[i] = vertices[i];
		}
        ...
        //glBufferDataARB(GL_ARRAY_BUFFER_ARB, sizeof(float)*verts.size()+sizeof(normals), 0, GL_STATIC_DRAW_ARB);
		glBufferDataARB(GL_ARRAY_BUFFER_ARB, sizeof(float)*verts.size()+sizeof(normals)+sizeof(colors), 0, GL_STATIC_DRAW_ARB);
        //glBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0, sizeof(vertices), vertices);                             // copy vertices starting from 0 offest
		glBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0, sizeof(float)*verts.size(), &verts[0]);                             // copy vertices starting from 0 offest //jjcao
        glBufferSubDataARB(GL_ARRAY_BUFFER_ARB, sizeof(float)*verts.size(), sizeof(normals), normals);                // copy normals after vertices
        glBufferSubDataARB(GL_ARRAY_BUFFER_ARB, sizeof(float)*verts.size()+sizeof(normals), sizeof(colors), colors);  // copy colours after normals
    }
...
void initGL()
{
    ...
    glClearColor(0.5, 0.5, 0, 0);                   // background color
...
void displayCB()
{
    ...
    if(vboUsed) // draw cube using VBO
    {
		...
		//glNormalPointer(GL_FLOAT, 0, (void*)sizeof(vertices));//jjcao
		glVertexPointer(3, GL_FLOAT, 0, 0);
        glNormalPointer(GL_FLOAT, 0, (void*)(sizeof(float)*verts.size()));//jjcao
        //glColorPointer(3, GL_FLOAT, 0, (void*)(sizeof(vertices)+sizeof(normals)));        
		
		glColor4f(1.0,0.0,0.0,0.0);
        //glDrawArrays(GL_QUADS, 0, pointNum);		
		glPointSize(5.0);
