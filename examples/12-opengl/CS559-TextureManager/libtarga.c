...
void * tga_load( const char * filename, 
    ...
    ubyte  idlen = 0;               // length of the image_id string below.
    ubyte  cmap_type = 0;           // paletted image <=> cmap_type
    ubyte  image_type = 0;          // can be any of the IMG_TYPE constants above.
    uint16 cmap_first = 0;          // 
    uint16 cmap_length = 0;         // how long the colormap is
    ubyte  cmap_entry_size = 0;     // how big a palette entry is.
    uint16 img_spec_xorig = 0;      // the x origin of the image in the image data.
    uint16 img_spec_yorig = 0;      // the y origin of the image in the image data.
    uint16 img_spec_width = 0;      // the width of the image.
    uint16 img_spec_height = 0;     // the height of the image.
    ubyte  img_spec_pix_depth = 0;  // the depth of a pixel in the image.
    ubyte  img_spec_img_desc = 0;   // the image descriptor.
