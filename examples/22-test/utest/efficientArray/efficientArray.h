...
public:
	EfficientArray():m_data(0),m_capacity(0),m_size(0){} // default constructor
	...
	~EfficientArray(){ delete[] m_data;} // deconstructor
...
private:
	int reserve(int num, double value=0.0);		// 为数组增加内存空间，不能比现在的空间小，新增的元素的值设置缺省值0即可，返回m_capacity。备注：m_size不变
