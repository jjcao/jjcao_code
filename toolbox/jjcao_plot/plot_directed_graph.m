function plot_directed_graph(A,xy,varargin)
%plot_directed_graph Plot a Directed Graph
% plot_directed_graph(A,XY) Plots the Directed Graph represented by adjacency
%   matrix A and points xy using the default style described below
% plot_directed_graph(A,XY,PARAM1,VAL1,PARAM2,VAL2,...) Plots the Directed Graph
...
%       A = round(rand(n));
%       xy = [cos(t); sin(t)]';
%       plot_directed_graph(A,xy,'LineWidth',2,'MarkerSize',8);
%
%   Example:
...
%       A = round(rand(n));
%       xy = [cos(t); sin(t)]';
%       plot_directed_graph(A,xy,'Color',[0.67 0 1],'LineStyle',':');
%
% See also: gplot, plot
%
% Author: Joseph Kirk
