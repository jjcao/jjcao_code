...
function [phi,boundary,disp_img,frames] = superpixels(img, numSuperpixels, display_int, contour_color, seeds)
    ...
    if (nargout < 4)
        phi = evolve_height_function_N(img, timeStep, maxIterations, 'superpixels', display_int, [], numSuperpixels, seeds);
    else
        [phi,frames] = evolve_height_function_N(img, timeStep, maxIterations, 'superpixels', display_int, [], numSuperpixels, seeds);
    end
