...
%
%   Changed by JJCAO, 2010.
%
%   References:
%       [1] Movellan, J. R. - Tutorial on Gabor Filters. Tech. rep., 2002.

function [G,GABOUT]=gaborfilter(I,S,F,W,P,options)
options.null = 0;
if isa(I,'double')~=1
    I=double(I);
end

size = getoptions(options, 'size', fix(1.5/S));
...
% % G1=k.*exp(-pi*S^2.*(x.*x+y.*y)).*...
% % (exp(j.*(2*pi*F.*(x.*cos(W)+y.*sin(W))+P))-exp(-pi*(F/S)^2+j*P));
% R1=k.*exp(-pi*S^2.*(x.*x+y.*y)).*...
%     (cos(2*pi*F.*(x.*cos(W)+y.*sin(W))+P) - exp(-pi*(F/S)^2)*cos(P));
% I1=k.*exp(-pi*S^2.*(x.*x+y.*y)).*...
% (sin(2*pi*F.*(x.*cos(W)+y.*sin(W))+P) - exp(-pi*(F/S)^2)*sin(P));
% 
% R=real(G);
% I=imag(G);
% max(max(R1-R))
